...
class Blinx:
    def __init__(self, configs, i2c):
        self.sensors = []
        self.listSensorModify = {}
        for sensor, config in configs.items():
            newName = config['new']
            input = config['input']
            channels = config['channels']
            self.listSensorModify[newName] = {'name' : sensor, 'channel': {}, "config": {}, "input" : input}
            self.sensors.append(Sensor(sensor, channels,  input, i2c))
    ...
    def getTimeBuffer(self, time):
        ...
        return self.sensors[0].getTimeBuffer(time)
...
# Channels
class Channel():
    def __init__(self, error = bytearray(b'\xff\xfe')):
